-- XenoScanner v4.2.1  --  Rayfield Edition  --  Production  --  Safe re-execute
...
-- ============================================================
-- CHANGELOG
-- v4.2.1 Syntax Executor compatibility pass:
--   SX1: identifyexecutor shimmed in S0 (was missing, detection threw on Syntax)
--   SX2: IS_SYNTAX / IS_XENO / IS_SYNAPSE flags set after S0
--   SX3: S2 GUI parent: CoreGui test-write replaced with executor-flag cascade.
--        Writing a Frame to CoreGui wakes Roblox internal CoreGui scripts whose
--        dot-notation method calls (Instance.FindFirstChild) are rejected by
--        Syntax's strict Luau VM -> "Expected ':' not '.' calling member function"
--   SX4: destroyOld: CoreGui excluded from candidate list on Syntax
--   SX5: buildTree: CoreGui root skipped on Syntax (same wakeup risk as SX3)
--   task.cancel already shimmed in v4.2 -- no change needed
-- v4.2 Full Scan:
...
-- SX1: identifyexecutor shimmed -- used for executor-specific routing below.
-- Without this shim the detection cascade throws on any executor that does
-- not expose identifyexecutor as a global.
if not identifyexecutor then
    identifyexecutor = function() return "Unknown" end
end

-- SX2: executor detection flags -- set once, used throughout.
-- IS_SYNTAX gates all CoreGui-avoidance logic.
local EXECUTOR   = identifyexecutor():lower()
local IS_SYNTAX  = EXECUTOR:find("syntax")  ~= nil
local IS_XENO    = EXECUTOR:find("xeno")    ~= nil
local IS_SYNAPSE = EXECUTOR:find("synapse") ~= nil
    or (rawget(_G, "syn") ~= nil)
...
do
    ...
    if stored ~= nil and hookmetamethod then
        ...
        print("[XenoScanner v4.2.1] Restored __namecall from crashed prior session.")
    end
end
...
-- ============================================================
-- S2  GUI PARENT
-- SX3: The old approach test-wrote a ScreenGui to CoreGui inside a pcall
-- to detect whether CoreGui was writable. On Syntax this wakes Roblox's
-- internal CoreGui modules (GetFFlagBubbleVoiceIndicatorSetting etc.) which
-- contain dot-notation method calls that Syntax's strict Luau VM rejects,
-- producing "Expected ':' not '.' calling member function FindFirstChild".
-- Fix: route by executor identity with no side-effect probing.
--   Xeno / Synapse  --> CoreGui  (high-trust executors that own CoreGui)
--   Syntax          --> PlayerGui (runs at lower trust, must not touch CoreGui)
--   gethui present  --> gethui() result
--   fallback        --> PlayerGui
-- ============================================================
local GUI_PARENT
do
    if IS_XENO or IS_SYNAPSE then
        GUI_PARENT = CoreGui
    elseif IS_SYNTAX then
        -- SX3: do NOT write to CoreGui on Syntax
...
-- SX4: CoreGui excluded from candidate list on Syntax -- even
-- FindFirstChild on CoreGui can wake internal scripts on Syntax.
-- ============================================================
...
local function destroyOld()
    ...
    -- SX4: only include CoreGui in scan candidates on executors that can
    -- safely read it without waking Roblox internal CoreGui scripts.
    local parents = {}
    if not IS_SYNTAX then
        parents[#parents + 1] = CoreGui
    end
...
local function buildTree()
    ...
    ins("XENO GAME SCANNER v4.2.1  --  Full Hierarchy")
    ...
    for _, svcName in ipairs(C.ROOTS) do
        -- SX5: On Syntax, scanning CoreGui's descendants triggers Roblox's
        -- internal CoreGui scripts, reproducing the same strict-Luau VM crash.
        -- Skip it here; the PROPERTIES tab is still available for direct inspection.
        if IS_SYNTAX and svcName == "CoreGui" then
            ins(">> CoreGui  [skipped on Syntax -- use Properties tab to inspect]")
            ins("")
        else
...
local function buildRemoteList()
    ...
    for _, svcName in ipairs(C.ROOTS) do
        -- SX5: Skip CoreGui scan on Syntax for the same reason as buildTree.
        if IS_SYNTAX and svcName == "CoreGui" then
            -- skip silently -- no remotes live here that aren't accessible elsewhere
        else
            local ok, svc = pcall(function() return game:GetService(svcName) end)
...
local function doScanTree()
    ...
    if f ~= "" then statusStr = statusStr .. "  |  filtered" end
...
local function doFullScan()
    ...
    parts[#parts + 1] = SEP
    parts[#parts + 1] = "XENO GAME SCANNER v4.2.1  --  FULL GAME SCAN"
    ...
    if not ok2 then
        ...
        if UI.RemoteScanLabel then
            pcall(function()
                UI.RemoteScanLabel:Set("Remotes found: " .. #ST.remoteList .. "  (via Full Scan)")
            end)
        end
    ...
    if not ok3 then
        ...
        if UI.ScriptScanLabel then
            pcall(function()
                UI.ScriptScanLabel:Set("Scripts found: " .. #ST.scriptList .. "  (via Full Scan)")
            end)
        end
...
startSpy = function()
    ...
    -- N3: capture BEFORE hook closure is created -- no upvalue timing race.
        ...
        while ST.spyActive and ST.spyGen == myGen do
            if ST.spyDirty and #ST.spyLines ~= lastCount then
                ...
                if UI.SpyCountLabel then
                    pcall(function() UI.SpyCountLabel:Set("Calls captured: " .. lastCount) end)
                end
...
local _MAIN_OK, _MAIN_ERR = pcall(function()
    ...
    UI.Window = Rayfield:CreateWindow({
        Name                   = "XENO GAME SCANNER  v4.2.1",
        Icon                   = "radar",
        LoadingTitle           = "XenoScanner v4.2.1",
    ...
    UI.StatusLabel = InfoTab:CreateLabel("XenoScanner v4.2.1 -- Ready.")

    InfoTab:CreateSection("About")
    UI.InfoParagraph = InfoTab:CreateParagraph({
        Title   = "XenoScanner v4.2.1  --  Rayfield Edition",
        Content = "Backend: v4.2.1 (Syntax compat + rawget hook + _G persistence + pcall wrapper)\n"
    ...
    -- --------------------------------------------------------
    -- KEYBIND: RightShift toggles GUI visibility via UIS.
    -- ToggleUIKeybind in CreateWindow crashes on strict Rayfield builds.
    -- --------------------------------------------------------
...
if _MAIN_OK then
    setStatus("XenoScanner v4.2.1 -- executor: " .. EXECUTOR .. "  |  GUI: " .. tostring(GUI_PARENT))
    print("[XenoScanner v4.2.1 Rayfield] Loaded. Executor: " .. EXECUTOR
        .. " | GUI parent: " .. tostring(GUI_PARENT))
else
    print("[XenoScanner v4.2.1 Rayfield] FATAL INIT ERROR: " .. tostring(_MAIN_ERR))
end
