...
-- v4.1: Syntax Executor compatibility (6 fixes applied)
...
-- identifyexecutor MUST be shimmed: used for executor-specific routing.
-- ============================================================
...
-- FIX 3 (Issue 3): shim identifyexecutor so detection never throws
...
-- FIX 4 (Issue 4): executor detection flags -- set once, used throughout
local EXECUTOR   = identifyexecutor():lower()
local IS_SYNTAX  = EXECUTOR:find("syntax")  ~= nil
local IS_XENO    = EXECUTOR:find("xeno")    ~= nil
local IS_SYNAPSE = EXECUTOR:find("synapse") ~= nil or (rawget(_G, "syn") ~= nil)
...
-- ============================================================
-- S2  GUI PARENT
-- FIX 1 (Issue 1): The old approach test-wrote a Frame to CoreGui inside a
-- pcall to detect if CoreGui was writable. On Syntax this woke up Roblox's
-- internal CoreGui scripts (GetFFlagBubbleVoiceIndicatorSetting etc.) which
-- use dot-notation method calls that Syntax's strict Luau VM rejects, causing
-- "Expected ':' not '.' calling member function FindFirstChild" at line 1.
-- Fix: no side-effect probing. Route by executor identity instead.
--   Xeno / Synapse  --> CoreGui  (high-trust executors that own CoreGui)
--   Syntax          --> PlayerGui (Syntax runs at a lower trust context)
...
do
    if IS_XENO or IS_SYNAPSE then
        -- High-trust executors: CoreGui is the correct and safe parent
        GUI_PARENT = CoreGui
    elseif IS_SYNTAX then
        -- FIX 1: Syntax must NOT write to CoreGui -- use PlayerGui directly
        GUI_PARENT = LP:WaitForChild("PlayerGui")
    else
        -- Unknown executor: try gethui, fall back to PlayerGui
...
local function destroyOld()
    -- FIX 5 (Issue 4): only include CoreGui when executor can safely read it.
    -- On Syntax, even FindFirstChild on CoreGui can wake internal scripts.
    local parents = {}
...
local ST = {
    ...
    propsText      = nil,
    filterText     = "",
    -- FIX 2 (Issue 2): generation counter replaces task.cancel debounce.
    -- task.cancel does not exist on Syntax. Instead, each keypress increments
    -- filterGen. The task.delay callback captures its generation at creation
    -- time and silently exits if the generation has advanced past it.
    filterGen      = 0,
}
...
local function buildTree()
    ...
    for _, svcName in ipairs(C.ROOTS) do
        -- FIX 6 (Issue 5): On Syntax, scanning CoreGui's descendants triggers
        -- Roblox's internal CoreGui scripts, reproducing the same VM crash.
        -- Skip it on Syntax; the user can still inspect it via the PROPS tab.
        if IS_SYNTAX and svcName == "CoreGui" then
            ins(">> CoreGui  [skipped on Syntax -- use PROPS tab to inspect]")
            ins("")
        else
...
FilterInput:GetPropertyChangedSignal("Text"):Connect(function()
    ...
    -- FIX 2 (Issue 2): generation counter debounce -- no task.cancel needed.
    -- task.cancel does not exist on Syntax. Incrementing filterGen invalidates
    -- any in-flight task.delay without requiring executor API support.
    ST.filterGen = ST.filterGen + 1
    local myGen  = ST.filterGen
    task.delay(C.FILTER_DEBOUNCE, function()
        -- Only the most recent keypress generation runs the render
...
FilterClear.MouseButton1Click:Connect(function()
    -- Invalidate any in-flight debounce by advancing the generation
    ST.filterGen  = ST.filterGen + 1
    FilterInput.Text = ""
    ST.filterText    = ""
    applyFilter()
...
-- S17  INIT
-- ============================================================
setStatus("XenoScanner v4.1  --  executor: " .. EXECUTOR .. "  |  GUI parent: " .. tostring(GUI_PARENT))
print("[XenoScanner v4.1] Loaded. Executor: " .. EXECUTOR .. " | GUI parent: " .. tostring(GUI_PARENT))
