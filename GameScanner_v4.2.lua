-- XenoScanner v4.2 -- Production -- Pure ASCII -- Safe re-execute
-- Tabs: TREE | SCRIPTS | REMOTES | PROPS
-- Keybind: RightShift = toggle GUI
-- ============================================================
-- LOAD VIA BOOTSTRAP -- do NOT use loadstring(game:HttpGet(...))() directly.
-- Use Bootstrap.lua instead. See notes in S0.5 for why.
-- ============================================================
-- CHANGELOG v4.1 -> v4.2
-- N1: getrawmetatable shim added to S0
-- N2: S0.5 early hook recovery -- checks _G.__XenoScannerNamecallOrig and
--     restores the original __namecall via hookmetamethod BEFORE any
--     game:GetService() call. hookmetamethod is a direct C executor call
--     and does not go through __namecall, so it is safe even when the hook
--     is broken. This is the actual fix for the persistent crash.
-- N3: startSpy rewrites origNamecall capture to use rawget on the game
--     metatable BEFORE hookmetamethod is called. origNamecall is now
--     always populated when the hook closure first fires -- timing race
--     is structurally impossible. Persists original in _G for N2 recovery.
-- N4: stopSpy clears _G.__XenoScannerNamecallOrig on clean stop so
--     the next re-injection does not attempt a spurious restoration.
-- N5: Entire script body wrapped in pcall -- any crash during init now
--     prints a real error message with real context to executor console
--     instead of the opaque CoreGui wrapper attribution.
-- ============================================================
-- CHANGELOG v4.0 -> v4.1 (preserved)
-- H2: task.cancel() on spyRenderThread in stopSpy
-- S1: AutomaticCanvasSize=Y on ContentScroll and SourceScroll
-- S2: renderText preserves Y.Offset in CanvasSize write
-- S3: renderGeneration counter aborts stale chunk loops
-- C1: resolvePath GetService gated to i==2
-- C2: switchTab clears filter bar on every switch
-- ============================================================
...
if not getrawmetatable then
    -- Fallback: getmetatable works on unlocked tables; Roblox metatables are
    -- locked so this shim only matters on non-Roblox test environments.
...
-- ============================================================
-- WHY THIS EXISTS:
-- The spy system hooks game.__namecall via hookmetamethod. If that hook
-- ever installs with a broken origNamecall (nil), every namecall on game
-- crashes -- including game:GetService, game:HttpGet, everything. The
-- process-level hook persists across re-injections and across GUI destroys.
-- destroyOld() only kills the ScreenGui, not the metamethod.
--
-- This block runs before ANY game namecall (before S1 services).
-- hookmetamethod() is a direct executor C-call -- it does NOT go through
-- game.__namecall -- so it is safe to call even when __namecall is broken.
--
-- _G.__XenoScannerNamecallOrig is set by startSpy() in S12 before the hook
-- is installed. If the session ended cleanly (stopSpy called), the key is
-- cleared. If it persists here, it means the previous session crashed or
-- was abandoned with the hook live -- restore it now.
-- ============================================================
do
    ...
    if stored ~= nil and hookmetamethod then
        ...
        print("[XenoScanner v4.2] Restored __namecall from previous session.")
    end
end
...
-- S1  SERVICES
-- ============================================================
-- N5: entire script body from here to S17 is wrapped in pcall.
-- Any crash during init now prints a real error message with real
-- context to the executor console instead of Xeno's opaque
-- "CoreGui.[id]:1: attempt to call a nil value" attribution.
...
TitleLbl.Text               = "XENO GAME SCANNER  v4.2"
...
-- ============================================================
-- S12  REMOTE SPY
-- N3: origNamecall pre-captured via rawget(getrawmetatable(game), "__namecall")
--     BEFORE hookmetamethod is called. The closure is created with origNamecall
--     already populated -- the upvalue timing race is structurally impossible.
-- N4: _G.__XenoScannerNamecallOrig persists the original across re-injections
--     so S0.5 can restore it if the session ends without stopSpy being called.
-- H2: task.cancel on render thread in stopSpy.
-- ============================================================
...
local function startSpy()
    ...
    if not getrawmetatable then
        setStatus("[SPY] getrawmetatable unavailable -- cannot pre-capture namecall.")
        return
    end

    -- N3: capture origNamecall via rawget BEFORE the hook closure is created.
    -- The old pattern (local origNamecall; origNamecall = hookmetamethod(...))
    -- has a window between hook install and assignment where origNamecall is nil.
    -- Roblox fires internal namecalls constantly -- the window is always hit.
    -- rawget on the metatable reads the current __namecall synchronously with
    -- no hook involved, before any closure is created.
    local mt = getrawmetatable(game)
    if not mt then
        setStatus("[SPY] Could not access game metatable.")
        return
    end
    local origNamecall = rawget(mt, "__namecall")
    if not origNamecall then
        setStatus("[SPY] Could not read __namecall -- already hooked or unavailable.")
        return
    end

    -- N4: persist in _G. If this session crashes or is abandoned without stopSpy
    -- being called, S0.5 on the next re-injection will find this key and restore
    -- the original __namecall before any game service calls happen.
    ...
    -- Render loop: separate coroutine, never touches the hook thread.
    ...
    -- Install the hook. origNamecall is fully populated above -- no race possible.
...
local function stopSpy()
    ...
    -- N4: clear _G key -- hook restored cleanly, no recovery needed on next run
    _G.__XenoScannerNamecallOrig = nil

    -- H2: cancel render thread -- nil-ing reference alone leaves it alive 250ms
...
print("[XenoScanner v4.2] Loaded. GUI parent: " .. tostring(GUI_PARENT))

end) -- end pcall main body (N5)

-- N5: if anything during init crashed, print a real error + context
-- to the executor console so it is actually debuggable.
if not _MAIN_OK then
    print("[XenoScanner v4.2] FATAL INIT ERROR: " .. tostring(_MAIN_ERR))
end
